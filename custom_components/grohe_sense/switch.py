...
from . import BASE_URL, DOMAIN, GROHE_SENSE_GUARD_TYPE
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    for device in filter(
        lambda d: d.type == GROHE_SENSE_GUARD_TYPE, hass.data[DOMAIN]["devices"]
    ):
        entities.append(
            GroheSenseGuardValve(
                auth_session,
                device.locationId,
                device.roomId,
                device.applianceId,
...
class GroheSenseGuardValve(SwitchEntity):
    ...
    async def async_update(self):
        command_response = await self._auth_session.get(
            BASE_URL
            + f"locations/{self._locationId}/rooms/{self._roomId}"
            + f"/appliances/{self._applianceId}/command"
        )
        if (
            "command" in command_response
            and "valve_open" in command_response["command"]
        ):
            self._is_on = command_response["command"]["valve_open"]
        else:
            _LOGGER.error(
                "Failed to parse out valve_open from commands response: %s",
                command_response,
            )
    ...
    async def _set_state(self, state):
        ...
        if (
            ...
            _LOGGER.warning(
                "Got unknown response back when setting valve state: %s",
                command_response,
            )
