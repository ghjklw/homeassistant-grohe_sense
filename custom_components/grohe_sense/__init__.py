...
CONF_REFRESH_TOKEN = "refresh_token"  # nosec
...
GROHE_SENSE_GUARD_TYPE = (
    103  # Type identifier for sense guard, the water guard installed on your water pipe
)

GroheDevice = collections.namedtuple(
    "GroheDevice", ["locationId", "roomId", "applianceId", "type", "name"]
)
...
async def initialize_shared_objects(hass, refresh_token):
    ...
    for location in locations:
        ...
        for room in rooms:
            ...
            roomId = room["id"]
            appliances = await auth_session.get(
                BASE_URL + f"locations/{locationId}/rooms/{roomId}/appliances"
            )
...
class OauthSession:
    ...
    async def token(self, old_token=None):
        """Returns an authorization header. If one is supplied as old_token,
        invalidate that one"""
        ...
        refresh_response = await self._http_request(
            BASE_URL + "oidc/refresh", "post", headers=headers, json=data
        )
        if "access_token" not in refresh_response:
            _LOGGER.error(
                "OAuth token refresh did not yield access token! Got back %s",
                refresh_response,
            )
        else:
    ...
    async def post(self, url, json, **kwargs):
        return await self._http_request(
            url, method="post", auth_token=self, json=json, **kwargs
        )

    async def _http_request(
        self, url, method="get", auth_token=None, headers={}, **kwargs
    ):
        ...
        while True:
            ...
            try:
                async with self._session.request(
                    ...
                    _LOGGER.debug(
                        "Http %s request to %s got response %d",
                        method,
                        url,
                        response.status,
                    )
                    if response.status in (200, 201):
                        ...
                        if auth_token is not None:
                            _LOGGER.debug(
                                "Request to %s returned status %d, refreshing auth "
                                "token",
                                url,
                                response.status,
                            )
                            ...
                            _LOGGER.error(
                                "Grohe sense refresh token is invalid (or expired), "
                                "please update your configuration with a new refresh "
                                "token"
                            )
                            raise OauthException(response.status, await response.text())
                    else:
                        _LOGGER.debug(
                            "Request to %s returned status %d, %s",
                            url,
                            response.status,
                            await response.text(),
                        )
