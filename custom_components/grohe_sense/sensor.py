...
from . import BASE_URL, DOMAIN, GROHE_SENSE_GUARD_TYPE, GROHE_SENSE_TYPE
...
SENSOR_TYPES = {
    ...
    "flowrate": SensorType(
        VOLUME_FLOW_RATE_CUBIC_METERS_PER_HOUR, None, lambda x: x * 3.6
    ),
    "pressure": SensorType(PRESSURE_MBAR, DEVICE_CLASS_PRESSURE, lambda x: x * 1000),
    "temperature_guard": SensorType(
        TEMP_CELSIUS, DEVICE_CLASS_TEMPERATURE, lambda x: x
    ),
}
...
# The protocol returns notification information as a (category, type) tuple, this maps
# to strings
NOTIFICATION_TYPES = {
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    ...
    if DOMAIN not in hass.data or "devices" not in hass.data[DOMAIN]:
        _LOGGER.error(
            "Did not find shared objects. You may need to update your configuration "
            "(this module should no longer be configured under sensor)."
        )
        return
    ...
    for device in hass.data[DOMAIN]["devices"]:
        ...
        if device.type in SENSOR_TYPES_PER_UNIT:
            entities += [
                GroheSenseSensorEntity(reader, device.name, key)
                for key in SENSOR_TYPES_PER_UNIT[device.type]
            ]
            if (
                device.type == GROHE_SENSE_GUARD_TYPE
            ):  # The sense guard also gets sensor entities for water flow
                entities.append(
                    GroheSenseGuardWithdrawalsEntity(reader, device.name, 1)
                )
                entities.append(
                    GroheSenseGuardWithdrawalsEntity(reader, device.name, 7)
                )
        else:
...
class GroheSenseGuardReader:
    ...
    async def async_update(self):
        ...
        # XXX: Hardcoded 15 minute interval for now.
        # Would be prettier to set this a bit more dynamically based on the json
        # response for the sense guard, and probably hardcode so mething longer for the
        # sense.
        if datetime.now() - self._data_fetch_completed < timedelta(minutes=15):
            _LOGGER.debug(
                "Skipping fetching new data, time since last fetch was only %s",
        ...
        def parse_time(s):
            # XXX: Fix for python 3.6 - Grohe emits time zone as "+HH:MM", python 3.6's
            # %z only accepts the format +HHMM. So, some ugly code to remove the colon
            # for now...
            if s.rfind(":") > s.find("+"):
        ...
        measurements_response = await self._auth_session.get(
            BASE_URL
            + f"locations/{self._locationId}/rooms/{self._roomId}"
            + f"/appliances/{self._applianceId}/data?from={poll_from}"
        )
        if "withdrawals" in measurements_response["data"]:
            ...
            _LOGGER.debug(
                "Got %d new withdrawals totaling %f volume",
                len(withdrawals),
                sum(w["waterconsumption"] for w in withdrawals),
            )
            self._withdrawals += withdrawals
            if len(self._withdrawals) > 0:
                self._poll_from = max(
                    self._poll_from, self._withdrawals[-1]["starttime"]
                )
        elif self._type != GROHE_SENSE_TYPE:
            _LOGGER.info(
                "Data response for appliance %s did not contain any withdrawals data",
                self._applianceId,
            )
        ...
        if "measurement" in measurements_response["data"]:
            ...
            if len(measurements):
                ...
                self._poll_from = max(
                    self._poll_from, parse_time(measurements[-1]["timestamp"])
                )
        else:
            _LOGGER.info(
                "Data response for appliance %s did not contain any measurements data",
                self._applianceId,
            )
    ...
    def consumption(self, since):
        # XXX: As self._withdrawals is sorted, we could speed this up by a binary
        # search, but most likely data sets are small enough that a linear scan is fine.
        return sum(
            w["waterconsumption"] for w in self._withdrawals if w["starttime"] >= since
        )
...
class GroheSenseNotificationEntity(Entity):
            ...
            "\n".join(
                [
                    NOTIFICATION_TYPES.get(
                        (n["category"], n["type"]), "Unknown notification: {}".format(n)
                    )
    ...
    async def async_update(self):
        self._notifications = await self._auth_session.get(
            BASE_URL
            + f"locations/{self._locationId}/rooms/{self._roomId}"
            + f"/appliances/{self._applianceId}/notifications"
        )
...
class GroheSenseGuardWithdrawalsEntity(Entity):
    ...
    def state(self):
        if (
            self._days == 1
        ):  # special case, if we're averaging over 1 day, just count since midnight
            # local time
            since = (
                datetime.now()
                .astimezone()
